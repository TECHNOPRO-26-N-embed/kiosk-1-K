...
/* CP932文字列をUTF-8へ変換する。 */
static void cp932_to_utf8(const char *src, char *dst, size_t dst_size);

/* UTF-8優先で文字列をUTF-8へ正規化する。 */
static void normalize_to_utf8(const char *src, char *dst, size_t dst_size);
...
int load_products_from_csv(const char *filename) {
    ...
    while (fgets(line, sizeof(line), fp) != NULL && count < MAX_PRODUCTS) {
        Product p;
        trim_newline(line);
        normalize_to_utf8(line, normalized, sizeof(normalized));

        if (normalized[0] == '\0') {
            continue;
        }
        if (strncmp(normalized, "id,", 3) == 0) {
            continue;
        }

        if (sscanf(normalized, "%d,%63[^,],%d,%d,%d,%d",
                   &p.id,
                   p.name,
                   &p.price,
                   &p.stock,
...
/* CP932文字列をUTF-8へ変換する。失敗時は元文字列を返す。 */
static void cp932_to_utf8(const char *src, char *dst, size_t dst_size) {
#ifdef _WIN32
    int wide_len;
    WCHAR wide_buf[INPUT_BUF * 2];
    int utf8_len;
    ...
    wide_len = MultiByteToWideChar(932, MB_ERR_INVALID_CHARS, src, -1, wide_buf, (int)(sizeof(wide_buf) / sizeof(wide_buf[0])));
    if (wide_len <= 0) {
        snprintf(dst, dst_size, "%s", src);
        return;
    }

    utf8_len = WideCharToMultiByte(CP_UTF8, 0, wide_buf, -1, dst, (int)dst_size, NULL, NULL);
    if (utf8_len <= 0) {
        snprintf(dst, dst_size, "%s", src);
    }
#else
...
/* UTF-8ならそのまま、非UTF-8ならCP932としてUTF-8へ変換する。 */
static void normalize_to_utf8(const char *src, char *dst, size_t dst_size) {
    if (dst_size == 0) {
        return;
    }

    if (is_valid_utf8(src)) {
        snprintf(dst, dst_size, "%s", src);
        return;
    }

    cp932_to_utf8(src, dst, dst_size);
}
